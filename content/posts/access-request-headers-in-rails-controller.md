...
date = 2023-02-12T15:04:22-05:00
tags = ["ruby", "rails"]
...
They wrote the test exactly like I'd [expect](https://relishapp.com/rspec/rspec-rails/docs/request-specs/request-spec#requesting-a-json-response). But, rather than providing the 418, a 200 OK was the status code. I then looked at the controller this request spec was accessing.
...
Nothing *obvious* caught my attention. But now that I'd been effectively [nerd sniped](https://en.wikipedia.org/wiki/Nerd_sniping), I had to figure out what was going on.
...
The change to get our test to pass is small. We don't want the response header, which is the `headers` variable. We need the request headers, which are accessible at `request.headers`.
