...
date = 2024-03-22T07:00:22-05:00
tags = ["ruby"]
...
The jobs executed and the batch is complete. Note that the total number of jobs is five, even though we enqueued three jobs. Interesting! Let's leave that aside for now as we explore what to do now that we have a batch that completes.
...
If you really need to, you *could* work around this by changing how you enqueue the jobs in your batch. Using [bulk queueing](https://github.com/sidekiq/sidekiq/wiki/Bulk-Queueing) will get you the result you expect.
